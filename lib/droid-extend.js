...
DroidExtend.prototype.isValidExtension = function(droid, extension) {
  ...
  return new Promise(function(resolve, reject) {
    if (droid.extensions.indexOf(extension) !== -1) {
      return extend.removeExtension(name, extension)
        .then(function() {
    ...
    if (!coreFunctions) {
      return reject({error: 'functions_not_found', message: 'Could not find droid functions defined in the spec. Extension: ' + extension});
    }
...
DroidExtend.prototype.extend = function(droid, extension) {
  var spec = this.getExtensionFile(extension);
  var coreFunctions = this.getExtensionFile(extension, spec.script);
  ...
  return new Promise(function(resolve, reject) {
    ...
    listeners.forEach(function(listener) {
      if (aliases.indexOf(listener.value) !== -1) {
        droid.listener.remove(listener.id);
      }
    });
  });
};
