...
DroidExtend.prototype.extend = function(droid, extension) {
  ...
  return new Promise(function(resolve, reject) {
    try {
       ...
       * @Deprecated
       */
      if (typeof droidMethods !== 'function') {
        console.warn('DEPRECATED RETURN: Your droid\'s `' + extension + '` main script must now return a function');
        coreFunctions = droidMethods;
      } else {
        coreFunctions = droidMethods({
          config: env,
          gynoid: gynoid
    ...
    console.log('Extensions %s installed', extension);
    return resolve(droid);
  });
};
