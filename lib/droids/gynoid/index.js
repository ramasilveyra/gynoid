...
module.exports = {
      ...
      return matrix.registerDroid(droid, config.droids[droid].token, true)
        .then(function() {
          ...
          return Promise.map(config.droids[droid].extensions, function(extension) {
            delete require.cache[require.resolve(env.GYNOID_INSTALL_PATH + '/' + extension + '/droid.json')];
  ...
  installDroid: function(req, res) {
    ...
    installFromGitHub(repository)
      .then(function() {
        ...
        try {
          delete require.cache[require.resolve(folder + '/droid.json')];
      ...
      .then(function() {
        ...
        matrix.installDroid(spec, coreFunctions, token, repository.split('/')[1])
          ...
          .catch(function(err) {
            rimraf.sync(env.GYNOID_INSTALL_PATH + '/' + repository.split('/')[1]);
            winston.error(err);
            return res.text('Unable to install droid. \n\n```' + JSON.stringify(err) + '```').send();
          });
      })
      .catch(function(err) {
        winston.error(err);
  ...
  removeDroidExtension: function(req, res) {
    var droid = req.params.droid;
    ...
    if (!extension) {
      return res.text('Missing extension name').send();
    }

    return matrix.removeDroidExtension(droid, extension)
      .then(function() {
        return res.text('Droid extension ' + extension + ' removed').send();
      })
      .catch(function(err) {
        return res.text('An error has occurred when trying to unregister droid extension.' + '\n\n```' + JSON.stringify(err) + '```').send();
      });
  },
  reloadDroidExtension: function(req, res) {
    var gynoid = this;
    var droid = req.params.droid;
    var extension = req.params.repo.split('/')[1];
    ...
    return gynoid.removeDroidExtension(req, res)
      .then(function() {
        return gynoid.reloadDroid(req, res);
      })
      .then(function() {
        return gynoid.extendDroid(req, res);
      });
  },
  reloadDroid: function(req, res) {
    ...
    return matrix.unregisterDroid(droid, true)
      .then(function() {
        var config = matrix.configuration.read(env.GYNOID_CONFIG_PATH);
        return matrix.registerDroid(droid, config.droids[droid].token, true)
          .then(function() {
