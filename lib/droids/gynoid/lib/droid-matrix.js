...
DroidMatrix.prototype.registerDroid = function(botName, token, initialization) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    // droid.listener.set('help_generator', true);
    droid.listener.listen('help', function(req, res) {
      if (req.message.value.mentioned) {
        var listeners = droid.listener.getAllListeners();
        var aliases = listeners.map(function(l) {
          return l.value;
        });

        return res.text('Mentioned!\n' + aliases.join('\n')).send();
      } else {
        return res.text(JSON.stringify(req.message)).send();
      }
    });
...
DroidMatrix.prototype.unregisterDroid = function(botName, initialization) {
  var matrix = this;
  return new Promise(function (resolve, reject) {
    if (botName === 'gynoid') {
    ...
    async.parallel([
      ...
      function() {
        ...
        if (!initialization) {
          configuration.droids[botName].extensions.forEach(function(extension) {
            var targetDir = env.GYNOID_INSTALL_PATH + '/' + extension;
...
DroidMatrix.prototype.removeDroidExtension = function(botName, extension) {
  var matrix = this;
  return new Promise(function (resolve, reject) {
    if (botName === 'gynoid') {
      return reject({error: 'droid_system', message: 'Cannot unregister a system droid.'});
    ...
    var configuration = matrix.configuration.read(env.GYNOID_CONFIG_PATH);
    var extensionIndex = configuration.droids[botName].extensions.indexOf(extension);
    if (extensionIndex === -1) {
      return reject({error: 'extension_unknown', message: 'Could not find extension ' + extension + ' in the registry.'});
    }

    rimraf.sync(env.GYNOID_INSTALL_PATH + '/' + extension);
    configuration.droids[botName].extensions.splice(extensionIndex, 1);
    try {
      matrix.configuration.update(env.GYNOID_CONFIG_PATH, configuration);
    } catch(err) {
      return reject(err);
...
DroidMatrix.prototype.extendDroid = function(spec, core, folder) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    // If a folder is provided save its location to the configuration
    if (folder) {
      var configuration = matrix.configuration.read(env.GYNOID_CONFIG_PATH);
...
DroidMatrix.prototype.listDroids = function() {
  var matrix = this;
  return new Promise(function(resolve) {
    return resolve(Object.keys(matrix.droidRegistry));
  });
};
